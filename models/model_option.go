...
func (modelOption *ModelOption) GetAggregateOption(modelOptions []ModelOption) *ModelOptionMultipleSorting {
	...
	modelOptionsFormatted := new(ModelOptionMultipleSorting)
	if len(modelOptions) == 0 {
		modelOptionsFormatted.Sort = bson.D{primitive.E{Key: "sequence", Value: 1}}
		return modelOptionsFormatted
	}

	if modelOptions[0].Limit > 0 {
		modelOptionsFormatted.Limit = modelOptions[0].Limit
	}

	if modelOptions[0].Skip > 0 {
		modelOptionsFormatted.Skip = modelOptions[0].Skip
	}
	...
	modelOptionsFormatted.Sort = sorts
